...
In this scenario we have an already running Zeebe Partition, and we were Leader before. This means all our services have been already installed, and the Leader has executed the command processing and exporting. After switching to the Follower, we need to stop the processing, exporting and regularly sending of last exported positions. We need to disallow new incoming user commands, via disabling the Command API. One important part before we start with the Follower Replay Mode is that we need to restore the current state from the latest snapshot. This is necessary because when we process commands we immediately apply the follow-up events to our state, which we write to the log later. We can't be sure what we have already applied is committed, and to not apply events twice it is easier to just restore from the latest snapshot, KISS principle. Other then that it is similar to described [above](#bootstrap-follower-partition).
